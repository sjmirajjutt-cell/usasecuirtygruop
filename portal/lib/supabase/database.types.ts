...
export type Attendance = { id: string; employee_id: string; check_in: string; check_out: string | null; total_hours: number | null; assigned_location_id: string | null; check_in_latitude: number | null; check_in_longitude: number | null; check_in_accuracy_meters: number | null; distance_from_location_meters: number | null; location_status: 'matched' | 'outside_radius' | 'not_verified'; is_paid: boolean; notes: string | null; created_at: string }
export type AttendanceInsert = Omit<Attendance, 'id' | 'total_hours' | 'created_at' | 'is_paid'> & { id?: string; created_at?: string; is_paid?: boolean }
